...
function checkPasswordStrength(password) {
  ...
  const hasCommonPatterns =
    ...
    /abcde|bcdef|cdefg|defgh|efghi|fghij|ghijk|hijkl|ijklm|jklmn|klmno|lmnop|mnopq|nopqr|opqrs|pqrst|rstuv|stuvw|tuvwx|uvwxy|vwxyz/.test(
      passwordLower
    ) || // sequential letters
...
function validateEmailAddress(email) {
  ...
  if (!ALLOWED_DOMAINS.includes(domain)) {
    return {
      isValid: false,
      message:
        "Only emails from Gmail, Yahoo, Outlook, Hotmail, ProtonMail, and iCloud are accepted",
    };
  }
...
function validateForm() {
  ...
  if (!password) {
    ...
    if (strength === "weak") {
      showFieldError(
        "password",
        "Password Requirements: At least 12 characters with 2 uppercase letters (A–Z), 2 lowercase letters (a–z), 2 numbers (0–9), and 2 special characters (! @ # $ % ^ & * etc.). Only Strong or Very Strong passwords are allowed."
      );
      isValid = false;
    } else if (strength === "average") {
      showFieldError(
        "password",
        'Password rejected: Avoid personal info (username, email), dictionary words, or simple patterns like "1234" or "aaaa". Only Strong or Very Strong passwords are allowed.'
      );
      isValid = false;
    }
  ...
  if (typeof grecaptcha !== "undefined") {
    ...
    if (!recaptchaResponse) {
      ...
      console.log(
        "[Register] reCAPTCHA not completed - will use bypass token for testing"
      );
    }
  } else {
    console.log(
      "[Register] reCAPTCHA not loaded - will use bypass token for testing"
    );
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (generatePasswordBtn) {
    console.log(
      "[Debug] Generate password button found, adding event listener"
    );
    generatePasswordBtn.addEventListener("click", (e) => {
      ...
      if (passwordInput && confirmPasswordInput) {
        ...
        showSuccess(
          "Very Strong password generated! Make sure to save it safely."
        );
      } else {
